/* tslint:disable */
export const memory: WebAssembly.Memory;
export function __wbg_monolithoptions_free(a: number): void;
export function monolithoptions_new(): number;
export function monolithoptions_noCss(a: number, b: number): void;
export function monolithoptions_noFrames(a: number, b: number): void;
export function monolithoptions_noImages(a: number, b: number): void;
export function monolithoptions_noJs(a: number, b: number): void;
export function monolithoptions_isolate(a: number, b: number): void;
export function monolithoptions_silent(a: number, b: number): void;
export function monolithOfUrl(a: number, b: number, c: number): number;
export function monolithOfHtml(a: number, b: number, c: number, d: number, e: number): number;
export function __wbindgen_exn_store(a: number): void;
export function __wbindgen_malloc(a: number): number;
export function __wbindgen_realloc(a: number, b: number, c: number): number;
export const __wbg_function_table: WebAssembly.Table;
